...
RoamexEdgeImportDriver::RoamexEdgeImportDriver(
    Profile* profile,
...
RoamexEdgeImportDriver::~RoamexEdgeImportDriver() = default;

void RoamexEdgeImportDriver::Start(
  ...
  coordinator_ = std::make_unique<RoamexEdgeImportCoordinator>(
      app_data_root_, profile_, base::MakeRefCounted<ProfileWriter>(profile_),
      plan.carriers, keychain_for_testing_);
  coordinator_->Run(base::BindOnce(&RoamexEdgeImportDriver::OnCoordinatorDone,
                                   weak_factory_.GetWeakPtr()));
}

void RoamexEdgeImportDriver::OnCoordinatorDone(EdgeImportReport report) {
...
bool MaybeStartEdgeBrowserSideImport(
  ...
  auto driver = std::make_unique<RoamexEdgeImportDriver>(
      target_profile,
      AppDataRootFromEdgeProfilePath(source_profile.source_path), items,
      /*keychain_for_testing=*/nullptr);
  RoamexEdgeImportDriver* driver_raw = driver.get();
  ...
  driver_raw->Start(base::BindOnce(
      [](std::unique_ptr<RoamexEdgeImportDriver> owned, base::OnceClosure done,
