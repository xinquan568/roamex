...
#ifndef ROAMEX_BROWSER_IMPORTER_ROAMEX_EDGE_IMPORT_DRIVER_H_
#define ROAMEX_BROWSER_IMPORTER_ROAMEX_EDGE_IMPORT_DRIVER_H_
...
class RoamexEdgeImportDriver {
 public:
  RoamexEdgeImportDriver(Profile* profile,
                         base::FilePath app_data_root,
                         uint16_t items,
                         crypto::apple::KeychainV2* keychain_for_testing);
  RoamexEdgeImportDriver(const RoamexEdgeImportDriver&) = delete;
  RoamexEdgeImportDriver& operator=(const RoamexEdgeImportDriver&) = delete;
  ~RoamexEdgeImportDriver();
 ...
 private:
  ...
  std::unique_ptr<RoamexEdgeImportCoordinator> coordinator_;
  base::OnceCallback<void(EdgeImportReport)> done_;

  base::WeakPtrFactory<RoamexEdgeImportDriver> weak_factory_{this};
};

// The host hook (patch 0015). If `source_profile` is a Chromium-Edge source and
// kEdgeImport is enabled, starts a self-owned RoamexEdgeImportDriver for the
...
#endif  // ROAMEX_BROWSER_IMPORTER_ROAMEX_EDGE_IMPORT_DRIVER_H_
