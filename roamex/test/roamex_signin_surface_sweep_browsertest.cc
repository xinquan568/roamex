...
// the deadline. Same idiom as roamex_tab_strip_position_settings_browsertest.
...
class RoamexSigninSurfaceSweepTest : public InProcessBrowserTest {
 public:
  RoamexSigninSurfaceSweepTest() {
...
class RoamexSigninSurfaceSweepFlagOffTest : public InProcessBrowserTest {
 public:
  RoamexSigninSurfaceSweepFlagOffTest() {
...
IN_PROC_BROWSER_TEST_F(RoamexSigninSurfaceSweepTest,
...
IN_PROC_BROWSER_TEST_F(RoamexSigninSurfaceSweepTest,
  ...
  Profile& profile_b = profiles::testing::CreateProfileSync(
      profile_manager,
      profile_manager->user_data_dir().AppendASCII("RoamexSweepProfileB"));
  ...
  ASSERT_TRUE(content::ExecJs(tab_a, "document.cookie = 'roamex_probe=a';"));
  EXPECT_EQ("roamex_probe=a", content::EvalJs(tab_a, "document.cookie"));
...
IN_PROC_BROWSER_TEST_F(RoamexSigninSurfaceSweepTest,
  ...
  prefs->SetBoolean(roamex::prefs::kSigninOptionalEntryPoint, true);
  prefs->CommitPendingWrite();
}

IN_PROC_BROWSER_TEST_F(RoamexSigninSurfaceSweepTest, OptInReexposesOnRestart) {
  PrefService* prefs = browser()->profile()->GetPrefs();
  ASSERT_TRUE(prefs->GetBoolean(roamex::prefs::kSigninOptionalEntryPoint));
