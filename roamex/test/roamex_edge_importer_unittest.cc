...
// roam-15 (I-3.1): the RoamexEdgeImporter StartImport→bridge envelope, the
...
#include "roamex/utility/importer/roamex_edge_importer.h"
...
TEST(RoamexEdgeImporterTest, StartImportDeliversRequestedItemsOnly) {
  ...
  auto importer = base::MakeRefCounted<roamex::RoamexEdgeImporter>();
...
TEST(RoamexEdgeImporterTest, CreateImporterByTypeReturnsRoamexEdge) {
  ...
  EXPECT_TRUE(static_cast<bool>(
      static_cast<roamex::RoamexEdgeImporter*>(importer.get())));
}

TEST(RoamexEdgeImporterTest, SourceProfileTypeSurvivesIpcParamTraits) {
