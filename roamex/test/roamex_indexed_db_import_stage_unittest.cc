...
class RoamexIndexedDbImportStageTest : public testing::Test {
 protected:
  void SetUp() override {
  ...
  size_t RunImport() {
    RoamexIndexedDbImportStage stage(edge_.GetPath(), dest_.GetPath());
...
TEST_F(RoamexIndexedDbImportStageTest, CopiesFirstPartyStoreVerbatim) {
  ...
  EXPECT_FALSE(
      base::PathExists(dest_.GetPath()
                           .Append(FILE_PATH_LITERAL("IndexedDB"))
                           .Append(FILE_PATH_LITERAL("roamex-idb-stage-"))));
}

TEST_F(RoamexIndexedDbImportStageTest, NoClobberLeavesExistingStoreIntact) {
  const std::string id = StoreId("https://idp.test");
...
TEST_F(RoamexIndexedDbImportStageTest, RetriedImportIsIdempotentNoClobber) {
  const std::string id = StoreId("https://idp.test");
  SeedEdgeStore(id, "idb-bytes");
...
TEST_F(RoamexIndexedDbImportStageTest, LegacyLevelDbBlobPairPublishTogether) {
  const std::string id = StoreId("https://idp.test");
...
TEST_F(RoamexIndexedDbImportStageTest, MissingSourceImportsNothing) {
  EXPECT_EQ(0u, RunImport());
}
